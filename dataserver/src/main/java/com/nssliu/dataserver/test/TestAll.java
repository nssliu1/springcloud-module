...
public class TestAll {
    ...
    public void testdbes(){
        ...
        try{
            ...
            esInput.buildStruct(EsUtils.getClient(),clazz,"hp2","cc");
        }catch (Exception e){
    ...
    public void testdbesSaveData() throws ClassNotFoundException, IllegalAccessException, InstantiationException, NoSuchFieldException {
        ...
        esInput.saveDatas(EsUtils.getClient(),clazz,listdata);*/
       ...
       /* List<Object> listdata = new ArrayList<>();
        for(Map.Entry<String, Object> entry : jsonObject1.entrySet()){
            ...
            area_name.set(o,parentName);
            area_code.set(o,parentCode);
            date_date.set(o, entry.getKey());
            date_str.set(o,entry.getKey());entry.getValue();
            price.set(o,new Double(entry.getValue().toString()));
            listdata.add(o);
        }
        ...
        for(int i = 0;i<childs.size();i++) {
            ...
            //保存区县级别
            esInput.saveDatas(EsUtils.getClient(),clazz,listdata2);
        }
        ...
        /*try{
            ...
            esInput.saveDatas(EsUtils.getClient(),clazz,datas);
        }catch (Exception e){

        }*/
      /*  String url = "https://changchun.fangjia.com/evaluation/avgPriceData?cityName=%E9%95%BF%E6%98%A5&region=%E5%8D%97%E5%85%B3&districtName=%E4%B8%87%E7%A7%91%E5%9F%8E&type=4";
        //InputStream inputStream = HttpRequests.interfaceUtil(url);
        //System.out.println(inputStream.toString());
        String s = HttpRequests.sendGet(url);
        System.out.println(s);
        //Object parse = JSON.parse(s);
        JSONObject jsonObject = JSON.parseObject(s);
        //纯json有属性的
        Object cityMap = jsonObject.get("cityMap");
        Object topWeekUpDownRange = jsonObject.get("topWeekUpDownRange");
        System.out.println(topWeekUpDownRange);
        //纯json的无属性的
        JSONObject jsonObject1 = JSON.parseObject(cityMap.toString());
        //Set<Map.Entry<String, Object>> entries = jsonObject1.entrySet();
        for(Map.Entry<String, Object> entry : jsonObject1.entrySet()){
            System.out.println(entry.getKey() + ":" + entry.getValue());
        }
